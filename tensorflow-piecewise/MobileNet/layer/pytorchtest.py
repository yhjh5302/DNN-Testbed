...
import time, math, io, numpy as np
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for i, data in enumerate(testloader, 0):
            ...
            inference_took += time.time() - start
            print("inference:", time.time() - total_start)
            
            start = time.time()
            f = io.BytesIO()
            np.save(f, outputs, allow_pickle=True)
            f.seek(0)
            out = f.read()
            ...
            if i == 99:
                ...
                print("inference_took: ", inference_took / 100)
                break
            if i % 100 == 99:
